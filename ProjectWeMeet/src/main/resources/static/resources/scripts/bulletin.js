...
function createCommentElement(comment, positionClass) {
    ...
    xhr.onreadystatechange = () => {
        if (xhr.readyState === XMLHttpRequest.DONE) {
            if (xhr.status >= 200 && xhr.status < 300) {
                ...
                const existingComments = Array.from(commentContainer.getElementsByClassName('comment'));

                for (const comment of comments) {
                    let isNewComment = true;

                    for (const existingComment of existingComments) {
                        if (existingComment.dataset.commentIndex === comment.index.toString()) {
                            isNewComment = false;
                            break;
                        }
                    }
                    ...
                    if (isNewComment) {
                        const positionClass = comment['deleted'] ? 'comment-left' : 'comment-right';
                        const newComment = createCommentElement(comment, positionClass);
